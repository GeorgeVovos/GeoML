"""Encoding-ablation VQC for v09B (combined encodings).

Identical variational structure, post-net, optimiser, and training schedule
as v09/v06. The ONLY thing that varies is how the input is loaded into the
quantum register, selected by the ``encoding`` argument — but here every
encoding combines a base feature map (at layer 0) with data reuploading
(layers 1..N-1). See quantum_encodings.py.
"""
...
from quantum_encodings import ENCODINGS, AMPLITUDE_ENCODINGS, N_QUBITS  # noqa: E402
...
class EncodingVQC(nn.Module):
	"""v06-equivalent VQC with a swappable combined-encoding stage."""
	...
	def __init__(self, encoding: str = "data_reuploading"):
		...
		# All v09B encodings combine with reuploading, so they all carry
		# learnable encoding weights. The buffer branch is kept for safety
		# in case a future non-learnable encoding is added.
		if needs_enc:
...
def train_quantum_vqc(fold_data, encoding: str = "data_reuploading",
	...
	"""Same training procedure as v06/v09 — only the encoding changes."""
	...
	# Encodings whose layer-0 base is AmplitudeEmbedding need unit-norm inputs.
	if encoding in AMPLITUDE_ENCODINGS:
