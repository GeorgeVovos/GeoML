"""v09B combined-encoding ablation driver.

5-fold CV for each *combined* encoding on the same v06 GSE76809
preprocessing. Where v09 compared standalone encodings, v09B compares each
base encoding fused with data reuploading:

	angle_reup, dense_angle_reup, amplitude_reup, iqp_reup

against the standalone ``data_reuploading`` reference. Reports per-encoding
AUC mean/std and Wilcoxon + Nadeau-Bengio paired tests against the
data_reuploading baseline (so we can say whether *adding* a structured base
encoding to reuploading helps, hurts, or does nothing).
"""
...
sys.path.insert(0, str(_THIS))  # Ensure v09B is first for local imports
...
def main():
	...
	out_path = out_dir / "v09b_encoding_ablation.json"
